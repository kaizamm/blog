...
### 环境准备
实验环境python-virtualenv，安装 sudo yum install python-virtualenv 即可。用这个环境的好处，可在系统的python解释器中避免包的混乱和版本的冲突，为每个程序单独创建虚拟环境可以保证程序只能访问虚拟环境中的包，从而保持全局解释器的干净整洁，并且虚拟环境中不需要管理员权限。一旦安装完成，virtualenv实用工具就可以从常规账户中调用。
```
...
#block标签定义的元素可在衍生模版中修改。本例中，我们定义了名为head、title和body块。注意，title包含在head中。下面是基版的衍生模板
