...
对于kubernetes操作人员来讲，我不管你们什么前端、后端，都是deployment/pods、statefuset、job或是cronjob。
...
这个annotation是让appdeploymnet来控制
...
git clone 项目地址到本地，后进入example 目录

```bash
➜  examples git:(master) ✗ pwd
/Users/hugo/code/kubevela/docs/examples
➜  examples git:(master) ✗ cd appdeployment
➜  appdeployment git:(master) ✗ pwd
/Users/hugo/code/kubevela/docs/examples/appdeployment
➜  appdeployment git:(master) ✗ ls
README.md                    appdeployment-2-traffic.yaml cluster.yaml
appdeployment-1.yaml         appdeployment-2.yaml         gateway.yaml
➜  appdeployment git:(master) ✗ kubectl apply -f appdeployment-1.yaml
appdeployment.core.oam.dev/example-appdeploy created
➜  appdeployment git:(master) ✗ k get appdeployment
NAME                AGE
example-appdeploy   7s
```

应用appdeployment后，可以看到负载已经正确创建，且deployment有两个副本

```bash
➜  appdeployment git:(master) ✗ k get deploy
NAME         READY   UP-TO-DATE   AVAILABLE   AGE
testsvc-v1   2/2     2            2           91s
➜  appdeployment git:(master) ✗ k get po
...
```bash
➜  appdeployment git:(master) ✗ k get appdeployment  example-appdeploy -o jsonpath='{.spec}' | jq
{
  "appRevisions": [
    {
      "placement": [
